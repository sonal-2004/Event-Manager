  ...
  const fetchAllData = async () => {
    try {
      const [eventRes, registeredRes] = await Promise.all([
        ...
        user ? axios.get('/api/events/registered') : Promise.resolve({ data: { events: [] } }),
      ]);

      // ✅ Expect backend response in the form: { events: [...] }
      setEvents(eventRes.data?.events || []);
      setRegisteredEvents((registeredRes.data?.events || []).map(ev => ev.id));
    } catch (err) {
